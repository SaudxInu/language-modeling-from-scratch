# torchrun --nnodes=1 --nproc_per_node=2 cs336_systems/naive_ddp_benchmarking.py --model_size xl
import argparse
import timeit
import contextlib
...
def run(
    ...
    with cm_nsys("backward_comm"):
        ...
        dist.all_reduce(grads_buffer, op=dist.ReduceOp.SUM)
        grads_buffer /= int(os.environ["WORLD_SIZE"])

        grads = torch._utils._unflatten_dense_tensors(grads_buffer, grads)
