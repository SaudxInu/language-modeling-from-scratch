...
def main(cfg: Config) -> None:
    ...
    for i in (
        pbar := trange(
            cfg.training.train_steps, desc="Training", disable=not is_master_process
        )
    ):
        lr = get_cosine_lr(
            i,
        ...
        for micro_step_idx in range(cfg.training.gradient_accumulation_steps):
            if is_ddp:
                ...
                model.require_backward_grad_sync = (
                    micro_step_idx == cfg.training.gradient_accumulation_steps - 1
                )
        ...
        if i != 0 and i % cfg.training.eval_interval == 0 and is_master_process:
            ...
            if cfg.training.save_checkpoints:
                model_weights_output_path = (
                    cfg.paths.model_output / f"step_{i:010d}" / "model.pt"
                )
                ...
                # Write config:
                with open(
                    model_weights_output_path.parent / "model_config.json", "w"
                ) as f:
                    json.dump(model_config, f, indent=4)
